...
#request에 chat_id가 있으면 해당 chat_id에 해당하는 prompt를 가져옴
if st.experimental_get_query_params("chat_id"):  # Corrected query parameter name
    selected_chat_id = st.experimental_get_query_params("chat_id")  # Corrected query parameter name
    prompt = conn.query("SELECT prompt FROM chat WHERE chat_id = %s;", ttl=600, params=(selected_chat_id,))
...
if prompt is not None:
     st.session_state['context_chat'] = prompt
